...
const Video = ({ onSearch, subscribed, onSubscribeClick }) => {
  ...
  const videoTitles = data.map(obj => obj.video.title);
  ...
  return (
    <div className="container">
      <div className='row dv'>
        ...
        <div className="show-data-wrapper">
          ...
          {isDataVisible && (
            <div className="data">
              <div className="boxes">
                <div className='tofix boxses d-none d-sm-block col-2 ps-4 '>
                  ...
                  <ul className='mt-5 mb-5 flex-column'>
                    ...
                    <li className='my-3'><NavLink className='li d-flex' to="https://studio.youtube.com/channel/"><MdOutlineVideoSettings className='me-2' />Your video</NavLink></li>
                    <li className=''><NavLink className='li d-flex' to="https://www.youtube.com/playlist?list=WL"><BsAlarm className='me-2' />Watch later</NavLink></li>
      ...
      <div className="row g-0">
        <div className="box__video col-lg-8 mt-5">
          <iframe className="iframe mt-3" src={`https://www.youtube.com/embed/${id}`} title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" ></iframe>
          {data.find(obj => obj.video.videoId === id) ? (
            <h3 className="video__title">{data.find(obj => obj.video.videoId === id).video.title}</h3>
          ) : (
          ...
          <div className="row align-items-center mt-2">
            <div className="col-xl-6 align-items-center">
              {data.filter(obj => obj.video.videoId === id).map((obj) => (
                <div className="mb-3" key={obj.video.videoId}>
                  <div className="row">
                    <div className="col-12 col-sm-6 col-md-2 m-0 card__inner">
                      <img className="card__imgs" src={obj.video.thumbnails[0].url} alt={obj.video.title} />
                    </div>
                    <div className="col-12 col-sm-6 col-md-5 mt-3 mt-sm-0">
                      <div className="card-bod">
                        <h5 className="card-title"><a href={`https://www.youtube.com/${obj.video.author.canonicalBaseUrl}`} target="_blank">{obj.video.author.title}</a></h5>
                        <p className="card-text"><small className="text-body-secondary">276 тыс. подписчиков</small></p>
                      </div>
                    </div>
                    <div className="col-12 col-md-4 mt-3 mt-md-0">
                      <button className="border-2 border border-dark bg-black text-white rounded-5 px-2 py-1 w-100" onClick={handleClick}>
            ...
            <div className="col-xl-6 row align-items-center m-0">
              <div className="col-12 col-sm-6 col-lg-4 d-flex align-items-center mb-3 mb-sm-0">
                <button className="btn btn-link d-flex align-items-center me-2" onClick={handleLikeClick} style={{ color: liked ? 'blue' : 'black' }}>
                  <AiOutlineLike className="me-1" /> {likes}
                </button>|
                <button className="ms-2 btn btn-link d-flex align-items-center" onClick={handleDislikeClick} style={{ color: disliked ? 'blue' : 'black' }}>
                  <AiOutlineDislike className="me-1" /> {dislikes}
                </button>
              </div>
              <div className="col-12 col-sm-6 d-flex align-items-center">
                <button className="border-2 d-flex align-items-center flex-grow-1 border border-dark bg-black text-white rounded-5 px-2 py-1 me-2" onClick={handleShareClick}>
                  <AiOutlineShareAlt className="me-1" /> Поделиться
                </button>
                <button className="border-2 d-flex align-items-center flex-grow-1 border border-dark bg-black text-white rounded-5 px-2 py-1" onClick={handleDownloadClick}>
                  <AiOutlineDownload className="me-1" />  Скачать
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="col-lg-4 d-flex flex-column box__card">
          {
            filter.map((fetch, index) => (
              <div className='text-sm-start text-center' key={index}>
                <div className=" my-2">
                  <Link className=" row " to={`/card/${fetch.video.videoId}`}>
                    <div className="card__inner  col-md-4" >
                      <img className="mx-auto mx-sm-0" src={`${fetch.video.thumbnails[0].url}`} />
                    </div>
                    <div className="col-md-8">
                      <div className="card-bod">
                        <h5 className="card-title ">{fetch.video.title}</h5>
                      </div>
                    </div>
                  </Link>
                </div>
